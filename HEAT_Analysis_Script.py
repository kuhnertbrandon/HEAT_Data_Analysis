...
class HEAT_Analysis():
	...
	def glob_search_csv(self):
		...
		if 'Hack' in title or 'AC' in title:
			self.indicator = 0
		else:
			self.indicator = 1
		dirs = title +'\\'
	...
	def find_first_row(self,file_list):
	...
	def create_bigdf(self,file_list,start_line):
	...
	def create_limitdf(self,bigdf,title,dirs):
	...
	def append_limit_df_to_master(self,title,limit_df):
		#
	...
	def save_df_to_parquet(self,df_in,title,dirs):
		df_in.to_parquet(dirs + title + '_Reliability_'+ timestamp +  '.parquet',engine='pyarrow')

	def master_scatter(self,limit_df,new_df,x_ax,y_ax): ## On hold
	...
	def comparison_bar_plot_Hack(self,old_path,new_df,title,dirs):
		## Call N-drive
