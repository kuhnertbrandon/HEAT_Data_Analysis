...
class HEAT_Analysis():
	...
	def create_limitdf(self,coupon_type,rod_diameter,maker,material,coverlay,moduli):
		...
		for i in daq_list:
			...
			row = pd.DataFrame([[title,coupon_type,date,maker,coverlay,material,moduli,self.channel_list[j],daq_list[j],strain,res_start,cycle_res10p,p30for10_lim,self.instrum_serial,bend_current_val]],
